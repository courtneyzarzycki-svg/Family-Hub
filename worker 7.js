...
async function checkAndNotifyWithLog() {
  const result = await checkAndNotify();
  // Store result in Firebase for debugging
  try {
    await fetch(FIREBASE_URL + "/workerLog.json", {
      method: "PUT",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({result, time: new Date().toISOString()})
    });
  } catch(e) {}
  return result;
}
...
export default {
  async scheduled(event, env, ctx) {
    ctx.waitUntil(checkAndNotifyWithLog());
  },
  async fetch(request, env, ctx) {
    const result = await checkAndNotifyWithLog();
